...
function ConfirmModal({ title, message, confirmLabel, onConfirm, onCancel, requireType }: ConfirmModalProps) {
  ...
  return (
    ...
    >
      <div
        className="w-full max-w-[400px] bg-[#141414] border border-white/10 rounded-2xl p-6 shadow-[0_24px_64px_rgba(0,0,0,0.8)]"
      ...
      <div className="px-4 pt-4 pb-3 border-b border-white/[0.07]">
        ...
        {stats && (
          <div className="mt-3 grid grid-cols-3 gap-1.5">
            {[{ label: t("navbar.gameLabel"), val: stats.counts?.Game ?? 0 }, { label: t("navbar.triviaLabel"), val: stats.counts?.Trivia ?? 0 }, { label: t("navbar.skillLabel"), val: stats.counts?.["Skill Badge"] ?? 0 }].map(({ label, val }) => (
