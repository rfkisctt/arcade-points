...
export function LangProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    if (cacheVersion !== CACHE_VERSION) {
      ...
      sessionStorage.removeItem("arcade_last_sync");
      ...
      <header className="fixed top-0 left-0 right-0 z-20 bg-[#141414]">
        <div className="w-full bg-[#141414] border-b border-dashed border-white/[0.06]">
          <div className="max-w-[1200px] mx-auto px-6 py-[9px] flex items-center justify-center">
            <a href="https://go.cloudskillsboost.google/arcade" target="_blank" rel="noopener noreferrer" className="group flex items-center gap-2 text-[12px] text-[#FCAA26] hover:text-[#FFBB3D] transition-colors duration-200 min-h-[24px]" style={{ fontFamily: "'seasonSans', sans-serif", fontWeight: 500 }}>
            ...
            <nav className="flex items-center justify-between h-[52px]">
              ...
              <div className="hidden md:flex items-center gap-6 absolute left-1/2 -translate-x-1/2">
                {navItems.map(({ href, label }) => {
                  ...
                  return (
                    <Link key={href} href={href} className={`text-[13px] transition-colors duration-200 min-h-[24px] flex items-center ${isActive ? "text-white font-[600]" : "text-white/45 hover:text-white/75 font-[400]"}`} style={{ fontFamily: "'seasonSans', sans-serif" }}>
                      {label}
                    </Link>
                  );
                })}
              ...
              <div className="flex items-center gap-2 shrink-0">
                ...
                >
                  ...
                  {stars !== null && (
                    <span className="text-[11px] font-[600]" aria-hidden="true">
